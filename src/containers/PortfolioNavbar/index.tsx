...
function PortfolioNavBar(): ReactElement {
  ...
  return (
    <div className='w-full sticky top-0 z-50 bg-black flex flex-row justify-between items-center md:px-20 px-4 py-4 md:grid md:grid-cols-4'>
      ...
      <img src={hamburger} alt='hamburger' className='md:hidden flex text-[#2790C4]' onClick={handleClick} />
      <div
        className={` ${
          showDropDown
        ...
        { PortfolioNavBarItems.map(({ id, route, name , offset , duration}) => {
          return (
            <NavLinks
              ...
              offset={offset} 
              duration={duration}
              activeClass = "border-b-2 border-[#2790C4] text-blue"
              className = "bg-black text-sm text-center pb-4 text-white active:border-b-2 active:border-[#2790C4] active:text-[#2790C4] active:font-bold"
